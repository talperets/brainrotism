...
@app.post("/players/login") # would it actually log u in
...
@app.post("/players/register") # does it insert into database idk
def register(username: str = Body(..., embed=True), db: Session = Depends(get_db)):
    ...
    # Create user
    user = models.Player(username=username)
    db.add(user)
    db.commit()
    db.refresh(user)
...
@app.get("/characters/{character_id}") # tested
